...
export default function ShareReferralCard({
  ...
  function handleSocialShare(channel: string) {
    ...
    if (channel === "email")
      url = `mailto:?subject=Earn%20rewards%20with%20RewmoAI!&body=${encodeURIComponent(
        shareMsg
      )}`;
  ...
  return (
    <div className="w-full max-w-md mx-auto bg-[#072b33] shadow-2xl rounded-2xl p-6 mb-6 border border-[#15C5C1]">
      ...
      <p className="text-[#B6E7EB] mb-4">
        Invite friends to RewmoAI! Get{" "}
        <span className="font-semibold text-[#FF9151]">bonus rewards</span>{" "}
        every time someone joins with your link.
      </p>
      ...
      <p className="mt-2 text-[#B6E7EB] text-xs text-center">
        Track your shares and referrals in your dashboard.{" "}
