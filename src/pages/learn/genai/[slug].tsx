...
export default function GenAiLesson({ course }: { course: Course }) {
  const external = !!course.url && /^https?:\/\//i.test(course.url);

  return (
    <>
      <Head>
        <title>{course.title} • GenAI Foundations • RewmoAI</title>
      </Head>

      <div className="min-h-screen bg-[#003B49] text-white">
        <div className="mx-auto max-w-4xl px-4 py-10">
          <Link href="/learn" className="text-sm text-[#B6E7EB] hover:text-white">
            ← Back to Learn
          </Link>
